...
public class ItemPicker<T> {
    ...
    public ItemPicker<T> add(Collection<? extends T> items) { return add(items.stream()); }
    /*
     * This must be final to allow @SafeVarargs. It must be @SafeVarargs to avoid warnings.
     */
    @SafeVarargs public final ItemPicker<T> add(T... items) { return add(Arrays.stream(items)); }
    public ItemPicker() {}
    public ItemPicker(String title) { this.title = title; }
    @SafeVarargs public ItemPicker(String title, T... items) {
        this.title = title;
        add(items);
    }
    ...
    /*
     * Binding may be left null, because we can always fall back to storing labels.
     * It's an optional binding, because we want to apply custom fallback,
     * especially when the returned value is non-empty but not one of the options.
     */
    private OptionalBinding<T> binding;
    public ItemPicker<T> binding(OptionalBinding<T> binding) {
        ...
        if (this.binding != null)
            ...
            binding = new OptionalBinding<T>() {
                @Override public Optional<T> get() {
                    var bound = sbinding.get().orElse(null);
                    return items.stream().filter(v -> naming.apply(v).equals(bound)).findFirst();
                }
                @Override public void set(Optional<T> value) { sbinding.set(value.map(naming)); }
            };
        }
        T bound = binding.get().orElse(fallback);
        T current = items.contains(bound) ? bound : fallback;
     ...
     */
    @SafeVarargs public static <T> T pick(String title, T... items) { return new ItemPicker<T>().title(title).add(items).pick(); }
    /*
    ...
    public static <T> T pick(String title, Collection<T> items) { return new ItemPicker<T>().title(title).add(items).pick(); }
    public static <T> T pick(String title, Stream<T> items, Function<T, String> naming) {
        return new ItemPicker<T>().title(title).add(items).naming(naming).pick();
    }
    public static <T> T pick(String title, Stream<T> items) { return new ItemPicker<T>().title(title).add(items).pick(); }
}
