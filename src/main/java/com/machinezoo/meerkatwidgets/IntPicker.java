...
public class IntPicker {
    ...
    public IntPicker add(int... items) { return add(Arrays.stream(items)); }
    public IntPicker add(IntCollection items) { return add(items.toIntArray()); }
    public IntPicker range(int start, int end) { return add(IntStream.range(start, end)); }
    public IntPicker range(int end) { return range(0, end); }
    public IntPicker rangeClosed(int start, int end) { return add(IntStream.rangeClosed(start, end)); }
    public IntPicker() {}
    public IntPicker(String title) { this.title = title; }
    ...
    private OptionalIntBinding binding;
    public IntPicker binding(OptionalIntBinding binding) {
    ...
    public int pick() {
        ...
        var binding = this.binding != null ? this.binding : SiteFragmentBindings.bindOptionalString(title).encodeInt();
    ...
    public static int pick(String title, IntFunction<String> naming, int... items) { return new IntPicker().title(title).add(items).naming(naming).pick(); }
}
