...
public class EnumPicker<T extends Enum<T>> {
    ...
    public EnumPicker<T> add(Collection<? extends T> items) { return add(items.stream()); }
    @SafeVarargs public final EnumPicker<T> add(T... items) { return add(Arrays.stream(items)); }
    public EnumPicker() {}
    public EnumPicker(String title) { this.title = title; }
    ...
    public EnumPicker<T> binding(OptionalBinding<T> binding) {
    ...
    @SuppressWarnings("unchecked") public T pick() {
     ...
     */
    public static <T extends Enum<T>> T pick(String title, T fallback) { return new EnumPicker<T>().title(title).fallback(fallback).pick(); }
    /*
     * Enums have the advantage of type information, which we can use to maximum
     * if we derive title from enum's type name.
     */
    public static <T extends Enum<T>> T pick(T fallback) { return new EnumPicker<T>().title(fallback.getClass().getSimpleName()).fallback(fallback).pick(); }
}
